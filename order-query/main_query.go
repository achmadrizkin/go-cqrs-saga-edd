...
func main() {
	...
	startGrpcServerQuery(table, client)
}

func startGrpcServerQuery(table *mongo.Collection, client *mongo.Client) {
	s := grpc.NewServer()

	orderQueryRepo := repo.NewOrderQueryRepo(table)
	orderQueryUseCase := usecase.NewOrderQueryUseCase(orderQueryRepo)

	pb.RegisterOrderQueryServiceServer(s, &server.OrderQueryServer{
		UnimplementedOrderQueryServiceServer: pb.UnimplementedOrderQueryServiceServer{},
		OrderQueryUseCase:                    orderQueryUseCase,
	})
	reflection.Register(s)

	// gRPC server
	lis, err := net.Listen("tcp", config.Config("GRPC_PORT_QUERY"))
	if err != nil {
